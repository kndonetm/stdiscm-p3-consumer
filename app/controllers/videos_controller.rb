...
class VideosController < ApplicationController
  def index
    @videos = Dir.glob(Rails.root.join("consumer", "public/videos", "*.mp4")).map do |path|
      {
        name: File.basename(path)[32..-1],  # skip video hash
        filename: URI.encode_www_form_component(File.basename(path)), #added this
        
        url: "/#{URI.encode_www_form_component(File.basename(path))}",
  ...
  def stream
    ...
    path = Rails.root.join("consumer", "public/videos", filename)
