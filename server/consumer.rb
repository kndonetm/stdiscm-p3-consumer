...
class VideoServer 
    ...
    def initialize(settings)
        @port = settings["VIDEO_SERVER_PORT"].to_i
        @bound_addr = settings["BOUND_ADDR"]
        @video_directory = File.expand_path(settings["VIDEO_DIRECTORY"])
    end
    ...
    def handle_client(client)
        ...
        while raw_cmd = receive_command(client) do 
            ...
            if cmd["action"] == "sendFile" then
                ...
                FileUtils.mkdir_p(@video_directory) unless Dir.exist?(@video_directory)
                filepath = File.expand_path(File.join(@video_directory, cmd["filename"]))
